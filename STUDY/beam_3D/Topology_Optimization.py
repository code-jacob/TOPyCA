...
radius_end = radius * 0.9                  # used when p = 3

# relaxation = "no"
relaxation = "yes"
p_relax = 2                     # p_relax = 2, start relaxing when p = 2
if relaxation == "yes":
    relaxation_factor_end = 0.85  # 0.7 -> allow 30% density change in next iteration for each point
...
def stamping_constraint():
    ...
    df_min = df.loc[df.groupby(["COOR_X", "COOR_Y"])["COOR_Z"].idxmin()]
...
if time_previous == 1:
    ...
    p_step = Convergence_1.loc[Convergence_1['Time'] == time_previous, 'p_step'].values[0]
...
if relaxation == "yes" : density_prev = df["density"]
update_density()
volume_constraint()
update_density()
...
merged_Convergence['p_step'] = merged_Convergence['p_step'].apply(lambda x: f"{x:.3g}")
...
table_str = tabulate(merged_Convergence, showindex=False, floatfmt=".6E", headers='keys', numalign="center", tablefmt="mixed_grid")
print(table_str)
with open('./RESULTS/Convergence.txt', 'w', encoding='utf-8') as f:
    f.write(table_str)
